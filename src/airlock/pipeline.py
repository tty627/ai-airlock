...
_PROHIBITED_EXFIL_TASKS = (
    ...
    re.compile(
        r"(?:读取|收集|提取|导出|获取|复制).{0,50}"
        r"(?:凭据|凭证|密钥|私钥|访问密钥|API\s*密钥|密码|令牌|环境变量)"
        r".{0,50}(?:上传|发送|外传|传输|发布|提交).{0,50}"
        r"(?:https?://|webhook|外部(?:服务器|网站|端点|系统)?|服务器|网站|端点)",
    ),
    re.compile(
        r"(?:读取|收集|提取|导出|获取|复制).{0,50}"
        r"(?:上传|发送|外传|传输|发布|提交).{0,50}"
        r"(?:凭据|凭证|密钥|私钥|访问密钥|API\s*密钥|密码|令牌|环境变量)"
        r".{0,50}(?:https?://|webhook|外部(?:服务器|网站|端点|系统)?|服务器|网站|端点)",
    ),
    re.compile(
        r"(?:上传|发送|外传|传输|发布|提交).{0,50}"
        r"(?:凭据|凭证|密钥|私钥|访问密钥|API\s*密钥|密码|令牌|环境变量)"
