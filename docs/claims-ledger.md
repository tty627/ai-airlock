...
blocking orphan-pipe failure（C-WIN-03）。rc.3 的 Windows cold failure 也必须保留为 `FAIL`。以下项目
仍按实际情况保持 `PENDING / NOT_RUN / NOT_MEASURED`，但都不会把已观察到的 rc.4 `FAIL` 改回
`INCONCLUSIVE`：

- exact rc.4 在未预填 source-artifact cache 条件下的 source download/bootstrap，以及任务期网络测量；
- Windows empty-cache、network 与未执行的其余 timeout/fault 项；
...
C-WIN-03 已违反必需条件，因此 rc.4 Windows candidate 与 overall 均为 `FAIL`。C-WIN-02 的早期 subset
结果仍按范围保留；预填 cache、`NOT_MEASURED` network、其余 `NOT_RUN` fault、Qoder 缺失和 Intel
performance `NOT_RUN` 都是另外的未知项，不是 C-WIN-03 的失败原因。任何 post-rc.4 working-tree 结果
在绑定新 exact tag 与独立证据前只能写 `POST_RC4_FIX_UNTAGGED / VALIDATION_PENDING`，不得写成 rc.4
修复证据、新候选 PASS 或 Qoder/Intel evidence。
