...
| C-WIN-02 | rc.4 fresh-tag Windows：`REGRESSION SUBSET PASS / FULL MATRIX INCONCLUSIVE` | exact tag 下通过 PS5.1/PS7 独立 process-cold+warm health、中文+空格路径 analyze、固定 invalid/missing errors、cross-shell concurrent cold、残留进程 `0`；252 个 frozen known-fixture markers 遍历 26 个 stdout/stderr 输出面时观察到 `0` hits | checkout 外的脱敏报告 bundle；无 public URL；manifest 校验 `99/99`；顶层 `SHA256SUMS` 文件 SHA-256 `3f0a17919118a858a29724752b5e68807b15a7ebadddbfdd9d81fa521ef29f3b` | `v0.1.0-rc.4`；tag object `2a50625aa95443e328573704cf42e9c633621ffe`；commit `52a215727115f32937cb78561e88a63fdae5adf2`；tree `46bc0f55eed58b7234338d4ff4e32bc71c348f8a`；Windows PowerShell 5.1 / PowerShell 7；source-artifact cache 预填 | “cold”只是独立进程路径，不证明 empty-cache source download/bootstrap；network `NOT_MEASURED`，timeout/fault 剩余矩阵 `NOT_RUN`，因此 Windows full matrix 为 `INCONCLUSIVE`；marker 结果只限冻结值与命名面，不是通用零泄漏。Qoder 缺失/`NOT_RUN` 与 Intel performance `NOT_RUN` 是另外的 overall 缺口，不是 Windows 矩阵限制的直接原因 | 是，只能写 subset PASS + Windows full-matrix INCONCLUSIVE | 可选，须带限制 | 否 |
...
rc.4 已有 exact-tag scoped Python CI（C-CI-02）和 Windows regression-subset 证据（C-WIN-02）。
但 rc.3 的 Windows cold failure 必须保留为 `FAIL`，以下项目仍按实际情况保持
`INCONCLUSIVE / PENDING / NOT_RUN`：

- exact rc.4 在未预填 source-artifact cache 条件下的 source download/bootstrap，以及任务期网络测量；
- Windows timeout/fault 剩余矩阵与 full-matrix verdict；当前只能写 `SUBSET PASS / FULL MATRIX INCONCLUSIVE`；
- Intel AI PC 性能、设备选择、NPU/GPU 使用情况与命名设备上的 cold/warm latency；
...
- 脱敏 rc.4 Windows 报告的 public URL，以及 ModelScope / 研习社 URL、真实截图和最终视频。

C-WIN-02 的预填 source-artifact cache、`NOT_MEASURED` 网络和 `NOT_RUN` timeout/fault 剩余矩阵
直接限制 Windows 结论为 full-matrix `INCONCLUSIVE`。Qoder 缺失/`NOT_RUN` 和 Intel performance
`NOT_RUN` 是另外的整体准入缺口，因此项目 overall 也为 `INCONCLUSIVE`；不得把它们改写成
Windows full-matrix 未通过的直接原因。任何新 working-tree 结果如未绑定 exact tag 与独立证据，仍不得
写成 rc.4 CI、Windows full PASS 或 Qoder/Intel evidence。
