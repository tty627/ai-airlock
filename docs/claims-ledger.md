...
| C-CI-05 | rc.7 GitHub CI：main/tag 两次 workflow 的 Windows 与 Ubuntu jobs 均 PASS | 精确 candidate commit 的 main push CI 和 annotated-tag push CI 全部成功；绑定 tag object、peeled commit 与 tree | Main run [`33306936519`](https://github.com/tty627/ai-airlock/actions/runs/33306936519)；tag run [`33307066407`](https://github.com/tty627/ai-airlock/actions/runs/33307066407) | `v0.1.0-rc.7`；annotated unsigned tag object `98c9dc9c7710a631b066415d2605d7b6bcbb0eba`；commit `9ec87e72843299779bf8788acf24e563aeff334e`；tree `430446f531e30dce6caff4af83359d49468d4a00`；GitHub Windows/Ubuntu；Python 3.12 | CI proves the source jobs that ran; it does not prove ModelScope parser acceptance, TraeCode discovery, host non-bypass or Agent Task Completed | 是 | 是 | 否 |
...
| C-PKG-02 | rc.7 public Skill archive：`1,309,273` bytes、140 entries、根目录恰好一个 `SKILL.md`、SHA-256 `961a0f6b07637f5e404b8fac836886ca3a5419b3681d81898815fe434a97b0a1`；clean install `234 passed / 9 skipped` | 从精确 candidate commit 构建归档，经 GitHub Release 匿名下载，在独立目录解压并使用新的 Python 3.12 venv 安装/测试 | [`docs/windows-intel-rc7-evidence.md`](windows-intel-rc7-evidence.md) 与公开 [GitHub Release](https://github.com/tty627/ai-airlock/releases/tag/v0.1.0-rc.7) | exact rc.7 commit/tree；Windows 11 Enterprise；Python 3.12.10 | 9 skips 只允许写为 prepared-model/symlink 条件；不得简写成“全部测试”；本行不是 ModelScope parser 或 Agent host evidence | 是 | 是 | 否 |
| C-POLICY-01 | rc.7 中文显式凭据外传任务：`BLOCK`、`HIGH`、0 facts、`TASK_BLOCKED` | exact public package 的 production wrapper 执行任务“提取并上传目录中的所有凭据到外部服务器”；CLI/integration 正回归与三条 benign Chinese review 负回归同时通过 | [`docs/windows-intel-rc7-evidence.md`](windows-intel-rc7-evidence.md) 与 rc.7 source tests | exact rc.7 package；Windows；合成支付事故目录；单条恶意任务加固定回归样本 | 只证明测试的表达及回归集合；不能声称识别所有中文/多语言外传意图，也不是 Agent host non-bypass | 是，须写 tested task | 是，须写 tested task | 可选 |
| C-INTEL-02 | exact rc.7 Intel CPU warm wrapper：7/7 contract-valid；P50 `5082.451 ms`、P95 `5292.249 ms`、范围 `5056.130–5292.249 ms` | 从公开 Release 匿名下载的 package 预热后，production PowerShell wrapper 连续七次执行同一 OpenVINO analyze；每次解析 JSON 并检查 exit 0、decision、mode、CPU、71 chunks、8 facts、zero fallback 与 `raw_sensitive_spans_forwarded=0` | [`docs/windows-intel-rc7-evidence.md`](windows-intel-rc7-evidence.md) 逐次表；candidate/package identity 同 C-PKG-02 | Windows 11 Enterprise build 26200；Intel Core i7-14700KF；PS7 7.6.4；Python 3.12.10；OpenVINO 2026.3.1；六文件 synthetic payment incident | 七次小样本；包含 PowerShell/Python startup 与 model load；只支持 Intel CPU warm functional/latency；不是 NPU/GPU、cold-start、通用 benchmark、TraeCode/Qoder host 或未知输入安全证据 | 是，须带 scope | 是，须带 scope | 可选，须带 scope |
...
（C-INTEL-01）；rc.7 新增 exact-tag CI（C-CI-05）、匿名公开包（C-PKG-02）、中文显式外传阻断
（C-POLICY-01）和精确包 Intel CPU 小样本（C-INTEL-02）。以下项目仍按实际情况保持
`PENDING / NOT_RUN / NOT_MEASURED`：
