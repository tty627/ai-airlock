...
- 最终泄漏闸门检查结构化 Capsule 和待写审计事件；人类输出只由已通过闸门的结构化结果生成。错误消息固定且不回显输入。

## 非目标与剩余风险

v0.1 不保证发现未知格式 Secret、规避式自然语言注入、图片/PDF 中的数据或被支持文本编码之外的内容。默认词法相关性和可选 OpenVINO embedding 都不能单独证明真实 Agent 的诊断质量；当前 OpenVINO A/B 只是合成 micro-fixture，阈值也尚未用独立 held-out 数据验证。语义 Prompt Injection 分类器、宿主集成和真实 utility benchmark 属于后续里程碑，发布前仍需真实 Windows/Intel/Qoder 验收。
