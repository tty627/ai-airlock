...
> Last reviewed: 2026-09-05 (Asia/Shanghai; finals session implemented and Linux OpenVINO evidence recorded)
>
...
- **Implemented on the finals development branch:** frozen sanitized sessions, bounded follow-up evidence,
  a loopback owner/client interface and cited report validation. See [build evidence](docs/finals-build-evidence.md).
  Linux Xeon CPU full tests: **384 passed / 12 skipped**; real OpenVINO synthetic evidence retention
  **36/43 first round → 43/43 with scripted follow-ups**. Cumulative protocol cost increases on these short cases.
- **Still unverified:** actual Agent answer quality, autonomous follow-ups, raw-file access denied under the
  real host identity, Core Ultra and the finals Windows flow. An API restriction is not an OS boundary.
...
The finals development work adds runtime code and its own scoped evidence; existing tags and releases remain unchanged.
